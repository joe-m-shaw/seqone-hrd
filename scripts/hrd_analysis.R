...
collated_myriad_info[
  collated_myriad_info$myriad_r_number == "R22-0LW4",
  "myriad_hrd_status"
] <- "NEGATIVE"
...
seqone_reports_v1_1 <- seqone_report_files_v1_1 |>
  map(\(seqone_report_files_v1_1) read_seqone_report(
    file = seqone_report_files_v1_1,
    version = "1.1"
  )) |>
  list_rbind()
...
low_res_myriad_results <- read_excel(
  path =
...
dna_concentrations_mod <- dna_concentrations |>
  ...
  mutate(
    ...
    dilution_worksheet = case_when(
      as.character(date_submitted) %in% c(
        "2023-07-21",
        "2023-07-26",
        "2023-08-03"
      ) ~ "WS133557",
...
seqone_mod <- collated_seqone_info |>
  ...
  mutate(
    downsampled = ifelse(grepl(
      pattern = c("downsampl|_0.5"),
      x = sample_id
    ), "Yes", "No"),
...
join_tables <- seqone_mod |>
  ...
  left_join(
    kapa_data_collated |>
      select(
        shallow_sample_id, q_pcr_n_m, ts_ng_ul, total_yield, q_pcr_n_m,
...
Description <- c(
  "Seraseq controls (Myriad GI scores available online)",
...
compare_results <- join_tables |>
  ...
  mutate(
    ...
    outcome_binary = fct(outcome_binary, levels = c(
      consistent_text,
      inconsistent_text,
      inconclusive_text,
      "other"
...
metric_table_inputs <- rbind(
  add_version(v1.1_results[[2]], "SomaHRD v1.1"),
...
metric_table_samples <- rbind(
  add_version(
    v1.2_unique_sample_results[[2]],
    "SomaHRD v1.2"
  ),
  add_version(
    v1.2_unique_sample_results_filtered[[2]],
...
low_tumour_content_table <- compare_results |>
  ...
  filter(!duplicated(dlms_dna_number)) |>
  select(
    dlms_dna_number, low_tumour_fraction,
...
intra_run_table <- intra_run_samples |>
  select(
    sample_id, seqone_hrd_status, seqone_hrd_score, lga, lpc, coverage,
    robustness
  ) |>
  rename(
    "Sample ID" = sample_id,
...
inter_run_samples <- compare_results |>
  filter(version == "1.2" &
    ...
    !sample_id %in% grep(
      x = compare_results$sample_id, pattern = "b|c",
      value = TRUE
    )) |>
  filter(base::duplicated(dlms_dna_number, fromLast = TRUE) |
    base::duplicated(dlms_dna_number, fromLast = FALSE))
...
hrd_score_facet_plot <- ggplot(inter_run_samples, aes(
  x = worksheet,
  y = seqone_hrd_score
)) +
  geom_point(
...
hrd_score_reads_facet_plot <- ggplot(inter_run_samples, aes(
  x = million_reads,
  y = seqone_hrd_score
)) +
  geom_point(
...
biobank_control_results <- compare_results |>
  ...
  select(
    dlms_dna_number, seqone_hrd_status, lga, lpc,
...
downsampled_samples <- grep(
  pattern = "20103853b|20112141b",
...
downsampled_table <- join_tables |>
  ...
  select(
    dlms_dna_number, downsampled, seqone_hrd_score, seqone_hrd_status,
...
inter_run_mod <- inter_run_samples |>
  mutate(
    total_bp = (million_reads * 1000000) * read_length,
...
hrd_score_repeat_plot <- repeated_samples |>
  ...
  mutate(dlms_dna_number = (fct_reorder(
    .f = dlms_dna_number, .x = seqone_hrd_score,
    .fun = median
  ))) |>
...
ggplot(results_and_profile, aes(
  x = worksheet,
  y =
  )) +
...
ggplot(results_and_profile, aes(x = reorder(
  shallow_sample_id,
  coverage
), y = coverage)) +
  geom_jitter(aes(colour = telomere_copy_profile), size = 3, alpha = 0.6) +
...
diff_profile <- results_and_profile |>
  ...
  select(
    worksheet, sample_id, telomere_copy_profile,
...
version_comparison <- compare_results |>
  select(
    shallow_sample_id, dlms_dna_number, lga, lpc, version,
    seqone_hrd_score, seqone_hrd_status
  ) |>
  pivot_wider(
    id_cols = c(shallow_sample_id, dlms_dna_number),
    names_from = version,
    values_from = c(lga, lpc, seqone_hrd_score, seqone_hrd_status)
  ) |>
  mutate(
    lga_change = lga_1.2 - lga_1.1,
...
ggplot(version_comparison, aes(
  x = seqone_hrd_score_1.1,
...
results_and_profile |>
  filter(telomere_copy_profile == "Increased high" & worksheet %in% c(
    "WS135498",
    "WS134928"
  ))
...
position_check <- results_and_profile |>
  mutate(
    plate_y = sub(x = plate_position, pattern = "\\d", replacement = ""),
...
control_check <- results_and_profile |>
  ...
  select(
    shallow_sample_id, dlms_dna_number,
