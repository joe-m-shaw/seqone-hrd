...
```{r}
#| label: export-collated-data
#| include: false

write.csv(all_hrd_data_pdf,
          file = here::here("data/seqone_collated_data/seqone_collated_pdf_data.csv"),
          row.names = FALSE)
...
Here are the referring centres for samples tested in the live service since December 2023.
...
live_samples <- live_data_pdf$sample_id
...
```{r}
#| label: referring-centres-plot
#| echo: FALSE

ggplot(seqone_referring_centres_summary, aes(x = reorder(referring_centre,
                                                         desc(n)), y = n)) +
  geom_col() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(y = "Number of samples tested",
       x = "")
