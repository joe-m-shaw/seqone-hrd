...
Total replicates tested: **`r length(unique(all_hrd_data$sample_id))`**
...
Total samples tested: **`r length(unique(live_data$sample_id))`**
...
data_for_histogram_plot <- all_hrd_data |> 
...
all_hrd_data |> 
  rbind(ws140359_pdf_data) |> 
  ggplot(aes(x = worksheet, y = robustness)) +
  geom_jitter(pch = 21) +
  theme_bw() +
  theme(panel.grid = element_blank(),
          axis.text.x = element_text(angle = 90, vjust = 0.2)) +
  scale_y_continuous(limits = c(0, 1.01), breaks = c(0, 0.25, 0.50, 0.75,
                                                  0.85, 0.93, 1.00)) +
  labs(x = "", y = "Genomic Instability", 
       title = "WS140359 is an outlier for robustness") +
  geom_hline(yintercept = 0.93, linetype = "dashed") +
  geom_hline(yintercept = 0.85, colour = safe_red)
