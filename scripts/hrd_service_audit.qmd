...
all_data_mod |> 
  filter(seqone_hrd_status != "NON-CONCLUSIVE") |> 
  ggplot(aes(x = reorder(shallow_sample_id, seqone_hrd_score),
                         y = seqone_hrd_score)) +
  geom_point(pch = 21, aes(fill = error_bar_qc_check)) +
  scale_fill_manual(values = c(safe_red, "white")) +
  geom_errorbar(aes(ymin = hrd_score_min, ymax = hrd_score_max)) +
  theme_bw() +
  theme(legend.position = "bottom",
        axis.text.x = element_blank()) +
  labs(title = "10 samples have error bars which cross 0.5 threshold",
       y = "SeqOne HRD Score", x = "Sample") +
  geom_hline(yintercept = 0.5, linetype = "dashed")
