...
pansolid_results_mod <- pansolid_results |> 
  mutate(brca_status = case_when(
    ...
    genotype %in% brca_strings ~"BRCA variant",
    
    genotype == "Analysis failed" ~"Fail",
    
    genotype == "No pathogenic variant identified" ~"No variant",
...
hrd_score_brca_plot <- seqone_pansolid_join |> 
  filter(brca_status %in% c("BRCA variant", "No variant") &
