...
For this validation, there was a series of samples with known genomic instability scores and HRD statuses from the Myriad test, which were then tested via the SeqOne method.

I will use this series of samples as the training dataset for my model, using the "generalized linear models" function on this dataset.
...
I can then overlay the boundary line from the official SeqOne pipeline to show that it is slightly different to my boundary line, but not by much. My model is a bit more likely to call samples are HRD negative.
