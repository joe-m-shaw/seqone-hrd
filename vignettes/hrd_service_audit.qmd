...
collated_live_csv_data |> 
  filter(!is.na(score)) |> 
  mutate(hrd_score_category = case_when(
    
    score > 0.6 ~"Above 0.6",
    
    score <= 0.6 & score>= 0.4 ~"Between 0.4 and 0.6",
    
    score < 0.4 ~"Less than 0.4"
    )) |> 
  ggplot(aes(x = reorder(sample, score), y = score)) +
  geom_hline(yintercept = 0.6, linetype = "dashed") +
  geom_hline(yintercept = 0.4, linetype = "dashed") +
  geom_point(shape = 21, aes(fill = hrd_score_category),
              alpha = 0.7) +
  scale_fill_manual(values = c(safe_red, "#FF9966" , safe_blue)) +
